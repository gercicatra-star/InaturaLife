(() => {
  ...
  function buildCategoryChips(items){
    ...
    el.hidden = false;
    el.innerHTML = [
      `<div class="chip ${!state.category?'on':''}" data-cat="">Todas</div>`,
      ...cats.map(c => `<div class="chip ${state.category===c?'on':''}" data-cat="${escapeHtml(c)}">${escapeHtml(c)}</div>`)
    ].join("");
  ...
  function renderList(items){
    ...
    el.innerHTML = items.map(i => {
      ...
      return `
        <div class="card" data-id="${escapeHtml(i.id)}">
          <div class="cardTop">
            <div>
