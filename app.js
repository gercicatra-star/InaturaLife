(() => {
  ...
  const teaCategoryImage = {
    "Digestivo": "assets/img/cat_digestivo.webp",
    "Circulação": "assets/img/cat_circulacao.webp",
    "Calmantes": "assets/img/cat_calmantes.webp",
    "Imunidade": "assets/img/cat_imunidade.webp",
    "Femininos": "assets/img/cat_femininos.webp",
    "Chás especiais": "assets/img/cat_especiais.webp"
  };

  const supplementCategoryColor = {
    "Sementes": "#0ea5e9",
    "Farinhas e pós": "#f97316",
    "Essenciais": "#22c55e",
    "Intestino e fibras": "#a855f7",
    "Verdes e algas": "#14b8a6",
    "Imunidade": "#84cc16",
    "Sono, calma e foco": "#8b5cf6",
    "Fitness e recuperação": "#ef4444",
    "Magnésio": "#06b6d4"
  };

  const supplementCategoryImage = {
    "Sementes": "assets/img/sup_sementes.webp",
    "Farinhas e pós": "assets/img/sup_farinhas.webp",
    "Essenciais": "assets/img/sup_essenciais.webp",
    "Intestino e fibras": "assets/img/sup_fibras.webp",
    "Verdes e algas": "assets/img/sup_verdes.webp",
    "Imunidade": "assets/img/sup_imunidade.webp",
    "Sono, calma e foco": "assets/img/sup_sono.webp",
    "Fitness e recuperação": "assets/img/sup_fitness.webp",
    "Magnésio": "assets/img/sup_magnesio.webp"
  };
  ...
  function moduleForKey(key){
    return DATA.modules.find(m => m.key === key);
  }

  function imageForItem(item){
    if(item && item.image) return item.image;
    if(item && item.module === "chas" && item.category){
      return teaCategoryImage[item.category] || moduleForKey("chas")?.image || null;
    }
        if(item && item.module === "suplementos" && item.category){
      return supplementCategoryImage[item.category] || moduleForKey("suplementos")?.image || null;
    }
    return moduleForKey(item.module)?.image || null;
  }
  ...
  function cardColors(item){
    ...
    if(!bg){
      const mod = moduleForKey(item.module);
  ...
  function styleVarsForCard(item){
    ...
    const overlay = (c.fg === "#ffffff")
      ? `linear-gradient(135deg, ${rgba(c.bg,0.82)}, ${rgba(c.bg,0.62)})`
      : `linear-gradient(135deg, ${rgba("#ffffff",0.78)}, ${rgba("#ffffff",0.46)})`;
    const cardImg = img ? `url("${img}")` : "none";
    return `--cardBg:${c.bg};--cardFg:${c.fg};--cardMuted:${c.muted};--tagBg:${c.tagBg};--tagBorder:${c.tagBorder};--tagFg:${c.tagFg};--cardOverlay:${overlay};--cardImg:${cardImg};`;
  }
  ...
  function renderSheetHero(item){
    const mod = moduleForKey(item.module) || {accent:"#14b8a6"};
    const img = (item.image || imageForItem(item) || "assets/img/inner_general.webp");
    const bg = cardColors(item).bg;
    const fg = cardColors(item).fg;
    const overlay = (fg==="#ffffff") ? `linear-gradient(135deg, ${rgba(bg,0.88)}, ${rgba(bg,0.62)})` : `linear-gradient(135deg, rgba(255,255,255,.82), rgba(255,255,255,.50))`;
    return `
      <div class="block hero" style="background-image:${overlay}, url('${img}'); background-size:cover; background-position:center;">
        <div class="hTitle">${escapeHtml(item.title||"")}</div>
        <div class="hSub">${escapeHtml([labelModule(item.module), item.category].filter(Boolean).join(" • "))}</div>
      </div>
    `;
  }
  ...
  function openSheet(item){
    ...
    const body = renderSheetHero(item) + renderBlocks(item.blocks || []);
  ...
  function renderModuleHero(){
    const mod = moduleForKey(state.module) || {title: state.module, accent: "#14b8a6", image: null};
    const img = mod.image ? `url("${mod.image}")` : "none";
    const hero = $("moduleHero");
    if(!hero) return;
    const fg = "#ffffff";
    const muted = "rgba(255,255,255,.86)";
    hero.innerHTML = `
      <div class="moduleHeroCard">
        <div class="moduleHeroBg" style="--heroImg:${img};--heroOverlay: linear-gradient(135deg, rgba(2,6,23,.62), rgba(2,6,23,.22));">
          <div class="moduleHeroContent" style="--heroFg:${fg};--heroMuted:${muted};">
            <div class="moduleHeroTitle">${escapeHtml(mod.title)}</div>
            <div class="moduleHeroSubtitle">Explore conteúdo organizado em cards: rápido de ler, fácil de salvar e ótimo para consultar no dia a dia.</div>
            <div class="moduleHeroPills">
              <span class="pill"><span class="dot" style="background:${mod.accent}"></span>${escapeHtml(mod.title)}</span>
              <span class="pill">Toque em um card para abrir</span>
            </div>
          </div>
        </div>
      </div>
